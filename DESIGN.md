...
- **克制的翡翠 Restrained Emerald**：唯一强调色。它是"合格/达标"的视觉同义词——出现在哪，哪就是你当前该看的地方。同色相按对比度需要分三档取值：
  ...
  - **翡翠·填充 Emerald Fill** (#0b7a55；夜 #2fbd86)：白字所坐的实底——主按钮、食材卡"＋份量"按钮 hover 底、焦点外框。浅色下比装饰档压深一档，让白字达 AA。
  - **翡翠·文字 Emerald Text** (#0b7a55；夜 #4fd39f)：翡翠色的文字与标签——品牌点、达标读数、食材卡"＋份量"按钮文字。夜里提亮到 #4fd39f，以在暗纸面上达标。
...
- **翡翠洗 Emerald Wash** (#e6f5ee)：翡翠的极浅底，作为暗色模式的装饰面与预留底色，不承载信息（当前无实例）。
...
### Hierarchy
- **Display** (800, clamp(34px–58px), 1.12, -0.02em): 仅落地页 Hero 主标题。
...
- **Label** (700, 12px, 0.2em 字距): 仅页首 eyebrow 小标（两页各一处，中文）。
...
站点分两页、共用一张样式表：`index.html`（落地页）与 `dorm-fit.html`（工具页）。两页统一容器最大 **1100px** 居中，左右内距 `clamp(16px, 4vw, 48px)`；区块垂直节奏 `clamp(40px, 6vw, 72px)`。钉在顶部的只有一个 **60px 导航条**。

关键布局与断点（既有实现，移动端一律塌成单列）：
- **落地页 Hero**：单列（`.hero.solo`），满屏高度垂直居中，内容上限 820px；只有一句主张 + 一颗主按钮，**无配图**。
- **工具页页首**（`.page-intro`）：顶部内距 `clamp(32px, 5vw, 60px)`，eyebrow + 一句话 + 主/次按钮，随后立即是目标三卡。
- **每日目标**：三列 `repeat(3, 1fr)`，`≤700px` 塌为单列。
- **今日搭配**：桌面 `1fr / 360px`（已选食材 / 总量对照），`≤900px` 塌为单列。
- **导航锚点**：`≤760px` 隐藏，保留品牌；工具页品牌链接回落地页。
- **采购明细行**：`≤600px` 由四列压成两列。
- **移动端底部标签栏**（`.tabbar`）：`≤760px` 固定底部 56px，五项（目标、食材、搭配、体重、采购），替代隐藏的顶部锚点；图标用内联 SVG，无外部依赖。

桌面端优先使用 CSS Grid 而非 flex 百分比运算；全站可点击件（按钮、食物卡）在 `:active` 下沉 `scale(0.98)` 作为触感回执。**移动端所有交互目标最小 44×44px**（按钮、步进、删除、输入框）。
...
### Navigation
- **Style:** 60px 高，`backdrop-filter: blur(10px)`，底色为 82% 不透明的纸色，底部 1px 柔线。品牌 17px/800（工具页为链接，回落地页），锚点 14px 石墨色，hover 转墨色；`≤760px` 锚点隐藏，**改显示底部固定标签栏（`.tabbar`，5 项，内联 SVG 图标）**。落地页右侧是一颗 Small Ghost 入口按钮（开始搭配 / 继续搭配）。
...
- **Behavior:** 读数达标或超出时，数值行追加翡翠色"(已达标)"或"(已达标 +Ng)"字样——这是全站最重要的达标反馈，语气是肯定而非警告。填充动画 `transition: transform .25s`。
